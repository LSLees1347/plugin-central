...
#include <SteamSdk/Includes/steam/isteamgameserver.h>
#include <SteamSdk/Includes/steam/steamtypes.h>
...
FString FSteamSharedModule::GetSteamModulePath() const
{
	...
	TSharedPtr<IPlugin> SteamPlugin = PluginManager.FindPlugin(TEXT("SteamIntegrationKit"));
	if(!SteamPlugin.IsValid())
	{
		return FString();
	}
	FString PluginRoot = SteamPlugin->GetBaseDir();
	auto MainModulePath = FPaths::Combine(*PluginRoot, TEXT("Source/SteamSdk"), TEXT("redistributable_bin"));
	const FString STEAM_SDK_ROOT_PATH(TEXT("Binaries/ThirdParty/Steamworks"));
#if PLATFORM_WINDOWS
#if PLATFORM_64BITS
	return MainModulePath / TEXT("win64/");
#else
	return FString();
...
#if PLATFORM_64BITS
	return MainModulePath / TEXT("linux64/");
#else
	return MainModulePath / TEXT("linux32/");
#endif	//PLATFORM_64BITS
#elif PLATFORM_MAC
	return MainModulePath / TEXT("/osx/");
#else
	return FString();
...
{
	UE_LOG(LogSteamShared, Log, TEXT("Obtaining Steam Server Instance Handle"));
	if (SteamServerObserver.IsValid())
	{
		UE_LOG(LogSteamShared, Log, TEXT("Steam Server Instance Handle already exists"));
		return SteamServerObserver.Pin();
	}
	else
	{
		UE_LOG(LogSteamShared, Log, TEXT("Creating Steam Server Instance Handle"));
		// Create the original base object, and store our weakptrs.
		...
		if (BaseInstance->IsInitialized()) // Make sure the SteamAPI was initialized properly.
		{
			UE_LOG(LogSteamShared, Log, TEXT("Steam Server Instance Handle initialized"));
			// This is safe because we'll end up incrementing the BaseInstance refcounter before we go out of scope.
			// Thus the InstanceHandler will not be deleted before we return.
			return SteamServerObserver.Pin();
		}
		...
		{
			UE_LOG(LogSteamShared, Log, TEXT("Steam Server Instance Handle failed to initialize"));
			// We don't want to hold any references to failed instances.
...
void FSteamSharedModule::LoadSteamModules()
{
	...
	{
		UE_LOG(LogSteamShared, Warning, TEXT("Unable to load %s from path %s. Steam functionality will not work"), *SteamModuleFileName, *RootSteamPath);
		return;
	}
	else
	{
		UE_LOG(LogSteamShared, Display, TEXT("Loaded %s from %s"), *SteamModuleFileName, *RootSteamPath);
	}
	...
	FSteamInstanceHandlerBase()
{
	...
	if (SteamInitializer != nullptr && SteamInitializer->AreSteamDllsLoaded())
	{
		...
		auto  Response = SteamAPI_InitEx(&ErrorMsg);
		if (Response == k_ESteamAPIInitResult_OK)
		{
			UE_LOG(LogSteamShared, Verbose, TEXT("SteamAPI initialized"));
			bInitialized = true;
			return;
		}
		UE_LOG(LogSteamShared, Warning, TEXT("SteamAPI failed to initialize. SteamSDK Error: %hs. SIK Tip: If this is editor logs, then getting this is normal as steam is only set to run on Standalone or builds. "), ErrorMsg);
		return;
	}
	...
	FSteamInstanceHandlerBase()
{
	...
	SteamErrMsg ErrorMsg;
	ESteamAPIInitResult Result = SteamGameServer_InitEx(LocalServerIP, GamePort, QueryPort,
		(bVACEnabled ? eServerModeAuthenticationAndSecure : eServerModeAuthentication),
		TCHAR_TO_UTF8(*GameVersion), &ErrorMsg);
	...
	{
		UE_LOG(LogSteamShared, Warning, TEXT("Steam Dedicated Server API failed to initialize. Error: %hs"), ErrorMsg);
	}
